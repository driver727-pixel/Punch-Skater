...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const titleSize = Phaser.Math.Clamp(width * 0.06, 24, 38);
    const subtitleSize = Phaser.Math.Clamp(width * 0.024, 10, 14);

    const bg = this.add.image(width / 2, height / 2, 'cyber-bg');
    bg.setDisplaySize(width, height).setAlpha(0.65);

    this.add.rectangle(width / 2, height / 2, width, height, 0x070714, 0.32);

    this.add.text(width / 2, height * 0.12, 'CYBER JOUST', {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(titleSize),
      color: '#ff007f',
      stroke: '#00f0ff',
      strokeThickness: 5
    }).setOrigin(0.5);
    ...
    this.add.text(width / 2, height * 0.17, 'SKATEBOARD ARENA CLASH', {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(subtitleSize),
      color: '#ffea00'
    }).setOrigin(0.5);
    ...
    this.events.once('shutdown', () => {
      if (typeof this.unsubscribeScores === 'function') {
  ...
  setupBackgroundMusic() {
    if (!this.sound.get('cyber-music')) {
      try {
        this.sound.add('cyber-music', { loop: true, volume: 0.35 });
      } catch {
        return;
      }
    }
  ...
  drawCustomizerPanel(x, y, w, h) {
    const headerSize = Phaser.Math.Clamp(w * 0.04, 10, 13);
    const optionSize = Phaser.Math.Clamp(w * 0.028, 8, 10);
    const statSize = Phaser.Math.Clamp(w * 0.022, 7, 8);

    this.add.rectangle(x, y, w, h, 0x0a0a1a, 0.85).setStrokeStyle(3, 0x00f0ff);

    this.add.text(x, y - h / 2 + 25, 'RIDER PROFILE', {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(headerSize),
      color: '#00f0ff'
    }).setOrigin(0.5);
    ...
    this.colorText = this.add.text(controlX, y - optionSpacing, `COLOR: ${this.colors[this.selectedColorIdx].name.toUpperCase()}`, {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(optionSize),
      color: '#ffffff'
    }).setOrigin(0.5);
    ...
    this.deckText = this.add.text(controlX, y, `DECK: ${this.decks[this.selectedDeckIdx].toUpperCase()}`, {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(optionSize),
      color: '#ffffff'
    }).setOrigin(0.5);
    ...
    this.weaponText = this.add.text(controlX, y + optionSpacing, `WEAPON: ${this.weapons[this.selectedWeaponIdx].name.toUpperCase()}`, {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(optionSize),
      color: '#ffffff'
    }).setOrigin(0.5);
  ...
  drawLeaderboardPanel(x, y, w, h) {
    const headerSize = Phaser.Math.Clamp(w * 0.04, 10, 13);
    this.add.rectangle(x, y, w, h, 0x0a0a1a, 0.85).setStrokeStyle(3, 0xff007f);
  ...
  renderLeaderboardText() {
    ...
    this.highScores.forEach((row, idx) => {
      const name = (row.playerName || 'ANON RIDER').slice(0, 12).padEnd(12, '.');
      const score = row.score || 0;
      const rank = idx + 1;
      const rankColor = rank === 1 ? '#ffea00' : rank === 2 ? '#00f0ff' : '#ffffff';
      const txt = this.add.text(0, idx * 24, `${rank}. ${name} ${score}`, {
        fontFamily: '"Press Start 2P"',
        fontSize: '10px',
        color: rankColor
      }).setOrigin(0.5);
  ...
  createActionButtons(width, height) {
    const isPortrait = height > width;
    const buttonFont = labelSize(Phaser.Math.Clamp(width * 0.024, 10, 14));
    const buttonY = height * 0.85;

    const playBtn = this.add.rectangle(width * 0.28, buttonY, width * 0.38, 55, 0xff0055);
    playBtn.setStrokeStyle(3, 0xffffff);
    const playTxt = this.add.text(width * 0.28, buttonY, 'SOLO ARENA', {
      fontFamily: '"Press Start 2P"',
      fontSize: buttonFont,
      color: '#ffffff'
    }).setOrigin(0.5);
    ...
    const multiBtn = this.add.rectangle(width * 0.72, buttonY, width * 0.38, 55, 0x00f0ff);
    multiBtn.setStrokeStyle(3, 0xffffff);
    const multiTxt = this.add.text(width * 0.72, buttonY, 'SHARE ROOM LINK', {
      fontFamily: '"Press Start 2P"',
      fontSize: labelSize(Phaser.Math.Clamp(width * 0.022, 9, 12)),
      color: '#111122'
    }).setOrigin(0.5);
  ...
  copyRoomLink() {
    ...
    const onFailure = () => this.showToast('ROOM READY. SHARE FAILED ON THIS DEVICE.', '#ffea00');
  ...
  showToast(message, color) {
    const notify = this.add.text(this.scale.width / 2, this.scale.height - 30, message, {
      fontFamily: '"Press Start 2P"',
      fontSize: '11px',
      color
    }).setOrigin(0.5);
  ...
  playTick() {
    try {
      this.sound.play('sfx-clash', { volume: 0.15, rate: 1.5 });
    } catch {
